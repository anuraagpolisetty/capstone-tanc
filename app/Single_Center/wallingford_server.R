...
output$social_wallingford <- renderPlotly({
  if(input$wallingford_gauge == 1) {
    wallingford.date.data <- wallingford_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
  else if(input$wallingford_gauge == 2) {
    wallingford.date.data <- wallingford_cleaned_data %>%  filter(Batch == '2018-1' | Batch == '2018-2')
    date <- '2018'
  }
  else{
...
output$physical_wallingford <- renderPlotly({
  if(input$wallingford_gauge == 1) {
    wallingford.date.data <- wallingford_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$positive_wallingford <- renderPlotly({
  if(input$wallingford_gauge == 1) {
    wallingford.date.data <- wallingford_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$services_wallingford <- renderPlotly({
  if(input$wallingford_gauge == 1) {
    wallingford.date.data <- wallingford_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$independence_wallingford <- renderPlotly({
  if(input$wallingford_gauge == 1) {
    wallingford.date.data <- wallingford_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$general_wallingford <- renderPlotly({
  if(input$wallingford_gauge == 1) {
    wallingford.date.data <- wallingford_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$wallingford_timeplot <-  renderPlot({
  ...
  else if(input$wallingford_answer == 2) {
    time.data <- wallingford_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Take.better.care.of.my.health), mean2 = mean(Eat.meals.that.are.better.for.me), mean3 = mean(Have.more.energy), mean4 = mean(Am.more.physically.active))
    social.life.means <-  time.data %>% select(mean1, mean2, mean3, mean4) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }
  else if(input$wallingford_answer == 3) {
    time.data <- wallingford_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Feel.happier.or.more.satisfied.with.my.life), mean2 = mean(Have.something.to.look.forward.to.each.day))
    social.life.means <-  time.data %>% select(mean1, mean2) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }
  else if(input$wallingford_answer == 4) {
    time.data <- wallingford_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Know.where.to.ask.if.I.need.a.service.such.as.a.ride.to.a.doctor.or.an.aide), mean2 = mean(Have.learned.about.services.and.benefits))
    social.life.means <-  time.data %>% select(mean1, mean2) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }
  
  else if(input$wallingford_answer == 5) {
    time.data <- wallingford_cleaned_data %>% group_by(Batch) %>%  summarise(total_mean = mean(Feel.more.able.to.stay.independent))
  }
  
  else if(input$wallingford_answer == 6) {
    time.data <- wallingford_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Would.recommend.the.senior.center.to.a.friend.or.family.member), mean2 = mean(Feel.that.the.senior.center.has.had.a.positive.effect.on.my.life))
    social.life.means <-  time.data %>% select(mean1, mean2) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }

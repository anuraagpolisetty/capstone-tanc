...
gwp_cleaned_data <- cleaned_data %>% filter(SiteID == 'GWP')


output$gauge <- renderPlotly({
  
  gwp.data.2019 <- data.2019 %>% filter(SiteID == 'GWP')   
  GaugeChart(gwp.data.2019, OverallSatisfactionIndex, "all", "2019")
  
})

output$social_gwp <- renderPlotly({
  if(input$gwp_gauge == 1) {
    gwp.date.data <- gwp_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
  else if(input$gwp_gauge == 2) {
    gwp.date.data <- gwp_cleaned_data %>%  filter(Batch == '2018-1' | Batch == '2018-2')
    date <- '2018'
  }
  else{
...
output$physical_gwp <- renderPlotly({
  if(input$gwp_gauge == 1) {
    gwp.date.data <- gwp_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$positive_gwp <- renderPlotly({
  if(input$gwp_gauge == 1) {
    gwp.date.data <- gwp_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$services_gwp <- renderPlotly({
  if(input$gwp_gauge == 1) {
    gwp.date.data <- gwp_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$independence_gwp <- renderPlotly({
  if(input$gwp_gauge == 1) {
    gwp.date.data <- gwp_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
...
output$general_gwp <- renderPlotly({
  if(input$gwp_gauge == 1) {
    gwp.date.data <- gwp_cleaned_data %>%  filter(Batch == '2017-1' | Batch == '2017-2')
    date <- '2017'
  }
  ...
  GaugeChart(gwp.date.data, OverallSatisfactionIndex, 'all', date)
  
})

output$gwp_timeplot <-  renderPlot({
  if(input$gwp_answer == 1) {
    time.data <- gwp_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Do.more.volunteer.work), mean2 = mean(See.friends.more.often.make.new.friends))
    social.life.means <-  time.data %>% select(mean1, mean2) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }
  else if(input$gwp_answer == 2) {
    time.data <- gwp_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Take.better.care.of.my.health), mean2 = mean(Eat.meals.that.are.better.for.me), mean3 = mean(Have.more.energy), mean4 = mean(Am.more.physically.active))
    social.life.means <-  time.data %>% select(mean1, mean2, mean3, mean4) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }
  else if(input$gwp_answer == 3) {
    time.data <- gwp_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Feel.happier.or.more.satisfied.with.my.life), mean2 = mean(Have.something.to.look.forward.to.each.day))
    social.life.means <-  time.data %>% select(mean1, mean2) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }
  else if(input$gwp_answer == 4) {
    time.data <- gwp_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Know.where.to.ask.if.I.need.a.service.such.as.a.ride.to.a.doctor.or.an.aide), mean2 = mean(Have.learned.about.services.and.benefits))
    social.life.means <-  time.data %>% select(mean1, mean2) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }
  
  else if(input$gwp_answer == 5) {
    time.data <- gwp_cleaned_data %>% group_by(Batch) %>%  summarise(total_mean = mean(Feel.more.able.to.stay.independent))
  }
  
  else if(input$gwp_answer == 6) {
    time.data <- gwp_cleaned_data %>% group_by(Batch) %>%  summarise(mean1 = mean(Would.recommend.the.senior.center.to.a.friend.or.family.member), mean2 = mean(Feel.that.the.senior.center.has.had.a.positive.effect.on.my.life))
    social.life.means <-  time.data %>% select(mean1, mean2) %>% rowMeans()
    time.data$total_mean <- social.life.means
  }

output$bar <- renderPlot({
  ...
  if (input$select3 == "Do more volunteer work") {
    ...
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(See.friends.more.often.make.new.friends) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "Take better care of my health") {
    x_value <- filtered3$Take.better.care.of.my.health
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Take.better.care.of.my.health) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "Eat meals that are better for me") {
    x_value <- filtered3$Eat.meals.that.are.better.for.me
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Eat.meals.that.are.better.for.me) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "Have more energy") {
    x_value <- filtered3$Have.more.energy
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Have.more.energy) %>% mutate(perc = n / sum(n)) 
  }
    ...
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Feel.happier.or.more.satisfied.with.my.life) %>% mutate(perc = n / sum(n)) 
  }
    ...
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Have.something.to.look.forward.to.each.day) %>% mutate(perc = n / sum(n)) 
  }
    ...
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Know.where.to.ask.if.I.need.a.service.such.as.a.ride.to.a.doctor.or.an.aide) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "Feel more able to stay independent") {
    x_value <- filtered3$Feel.more.able.to.stay.independent
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Feel.more.able.to.stay.independent) %>% mutate(perc = n / sum(n)) 
  }
    ...
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Feel.that.the.senior.center.has.had.a.positive.effect.on.my.life) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "Learn new things") {
    x_value <- filtered3$Learn.new.things
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Learn.new.things) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "Have learned about services and benefits") {
    x_value <- filtered3$Have.learned.about.services.and.benefits
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Have.learned.about.services.and.benefits) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "Am more physically active") {
    x_value <- filtered3$Am.more.physically.active
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Am.more.physically.active) %>% mutate(perc = n / sum(n)) 
  }
    ...
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(Would.recommend.the.senior.center.to.a.friend.or.family.member) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "What is your zip code") {
    x_value <- filtered3$What.is.your.zip.code
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(What.is.your.zip.code) %>% mutate(perc = n / sum(n)) 
  }
  else if (input$select3 == "What is your estimated annual income") {
    x_value <- filtered3$What.is.your.estimated.annual.income
    filtered4 <- filtered3 %>% group_by(SiteID) %>% count(What.is.your.estimated.annual.income) %>% mutate(perc = n / sum(n)) 
  }
  #filtered3 <- rbind(filtered, filtered2)
